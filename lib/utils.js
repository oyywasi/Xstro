...
async function audioToBlackVideo(audioBuffer) {
 return new Promise(async (resolve, reject) => {
  ...
  let inputPath = path.join(os.tmpdir(), `input-${Date.now()}.${type.ext}`);
  let mp3Path = path.join(os.tmpdir(), `audio-${Date.now()}.mp3`);
  const outputPath = path.join(os.tmpdir(), `output-${Date.now()}.mp4`);

  fs.writeFileSync(inputPath, audioBuffer);

  if (type.mime !== 'audio/mpeg' && type.mime !== 'audio/ogg') {
   ffmpeg(inputPath)
    .toFormat('mp3')
    .on('error', (err) => {
    ...
    .on('end', async () => {
     inputPath = mp3Path; // Set inputPath to the converted mp3Path
     await createBlackVideo(inputPath, outputPath, resolve, reject);
    })
    .save(mp3Path);
  } else {
   await createBlackVideo(inputPath, outputPath, resolve, reject);
...
async function createBlackVideo(audioPath, outputPath, resolve, reject) {
 ffmpeg()
  .input(audioPath)
  .inputFormat('lavfi')
  ...
  .videoCodec('libx264')
  .on('error', (err) => {
   console.error('FFmpeg error while creating video:', err);
