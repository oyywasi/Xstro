...
class Handler {
 ...
 _patch(data) {
  ...
  this.bot = Boolean(data.key.id.startsWith('BAE5'));
  ...
  try {
   this.sudo = config.SUDO.split(',').startsWith(this.participant.split('@')[0]);
  } catch {
   this.sudo = false;
  }
 ...
 _processQuotedMessage(quoted) {
  ...
  this.reply_message = {
   ...
   bot: Boolean(quoted.key.id.includes('BAE5')),
 ...
 /**
  * Sends an interactive message with various button types and customizable content
  * @param {Object} params - The parameters for the interactive message
  * @param {string} params.jid - The recipient's JID (WhatsApp ID)
  * @param {Array<Object>} params.button - Array of button configurations
  * @param {Object} [params.header] - Header configuration
  * @param {Object} [params.footer] - Footer configuration
  * @param {Object} params.body - Body content
  * @param {Object} [params.options={}] - Additional options for message generation
  ...
  *   button: [
  *     { type: "reply", params: { text: "Click me" } },
  ...
  * });
  */
 async sendInteractive({ jid, button, header = {}, footer = {}, body, options = {} }) {
  const buttonTypeMap = {
   copy: 'cta_copy',
   url: 'cta_url',
  ...
  const buttons = button.map((btn) => ({
   name: buttonTypeMap[btn.type] || 'quick_reply',
  ...
  try {
   ...
   const generatedMessage = generateWAMessageFromContent(jid, messageContent, options);
 ...
 async download(message) {
  ...
  const fileExtension = extensions[mimeType] || 'bin',
   fileName = `${mediaType}_${Date.now()}_${crypto.randomBytes(4).toString('hex')}.${fileExtension}`;
