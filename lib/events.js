...
class Handler {
  ...
  if (data.message) {
   ...
   if (data.quoted) {
    ...
    this.reply_message = {
     ...
     text: qMessage?.conversation || qMessage?.extendedTextMessage?.text || '',
 ...
 async _asyncPatch() {
  if (this.participant) {
   const [dbSudo, dbBan] = await Promise.all([isSudo(this.participant), isBan(this.participant)]);
   this.sudo ||= dbSudo;
   this.isban = dbBan;
  }
  this.owner = this.fromMe || this.sudo;
  ...
  if (this.reply_message?.sender) {
   await this._asyncReplyPatch();
  }
 }
 async _asyncReplyPatch() {
  const replySender = this.reply_message.sender;
  const [replySudo, replyBan] = await Promise.all([isSudo(replySender), isBan(replySender)]);
  this.reply_message.sudo = replySudo;
  this.reply_message.isban = replyBan;
 }
