...
class Client {
 ...
 async handleMessages(m) {
  ...
  for (const msg of m.messages) {
   ...
   let userCommand = '';
   if (serialized.body?.startsWith(config.PREFIX)) {
    userCommand = serialized.body.slice(config.PREFIX.length).trim().split(/\s+/)[0].toLowerCase();
   }
   ...
   if (!commandFound && userCommand) {
    const suggestions = this.findSimilarCommands(userCommand, commands);
    if (suggestions.length > 0) {
     const userMention = `@${serialized.sender.split('@')[0]}`;
     const suggestionText = suggestions.map((s) => `*${s}*`).join(', ');
     await this.conn.sendMessage(serialized.from, {
      text: `${userMention} did you mean ${suggestionText}?`,
 ...
 async findSimilarCommands(userCommand, commands) {
  const similarities = [];

  for (const command of commands) {
   if (command.pattern) {
    const patternStr = command.pattern.toString();
    const match = patternStr.match(/\\s\*\((.*?)\)/);
    if (match) {
     const commandName = match[1].split('|')[0];
     const similarity = this.calculateSimilarity(userCommand, commandName);
     if (similarity >= 0.7) {
      similarities.push(commandName);
     }
    }
   }
   if (command.alias) {
    for (const alias of command.alias) {
     const similarity = this.calculateSimilarity(userCommand, alias);
     if (similarity >= 0.7) {
      similarities.push(alias);
     }
    }
   }
  }

  return [...new Set(similarities)];
 }
