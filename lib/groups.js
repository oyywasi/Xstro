class GroupHandler {
 constructor(conn) {
  ...
  this.defaultProfilePic = 'https://i.ibb.co/4m0dj6g/profile-circle.png';
 }
 ...
 async getProfilePicture(jid) {
  try {
   return await this.conn.profilePictureUrl(jid, 'image');
  } catch {
 ...
 async handleWelcome(groupId, newParticipants) {
  try {
   ...
   for (const participant of newParticipants) {
    ...
    messageData.pp = messageConfig.message.includes('@pp') ? await this.getProfilePicture(participant) : null;
    const formattedMessage = await this.formatMessage(messageConfig.message, messageData);
    if (formattedMessage) {
     if (messageData.pp) {
      await this.conn.sendMessage(groupId, {
       image: { url: messageData.pp },
 ...
 async handlePromotion(groupId, promotedParticipants) {
  try {
   ...
   for (const promotedUser of promotedParticipants) {
    const profilePic = await this.getProfilePicture(promotedUser);
    const userMention = `@${promotedUser.split('@')[0]}`;
    const announceMessage = `*👑 NEW ADMIN ALERT*\n\n${userMention} has been promoted to admin of *${groupName}*\n\nCongratulations! 🎉`;
 ...
 async handleDemotion(groupId, demotedParticipants) {
  try {
   ...
   for (const demotedUser of demotedParticipants) {
    const profilePic = await this.getProfilePicture(demotedUser);
 ...
 async formatMessage(template, data) {
  ...
  return template.replace(new RegExp(Object.keys(replacements).join('|'), 'g'), (match) => (match === '@user' ? `${replacements[match]}\u200B` : replacements[match]));
 }
