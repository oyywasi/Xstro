...
//   type: 'games',
//   usage: 'anagram [easy|medium|hard]',
//  },
//  async (message) => {
//   const jid = message.chat;
//   if (activeGameJid && activeGameJid !== jid) {
//    return await message.reply("There's already an active game in another chat. Please wait for it to finish.");
//   }

//   const difficulty = message.text?.toLowerCase().trim() || 'medium';

//   if (anagramGame.hasActiveGame(jid)) {
//    const currentGame = anagramGame.getGameState(jid);
//    return await message.reply(`There's already an active game in this chat!\n` + `Difficulty: ${currentGame.difficulty}\n` + `Scrambled word: ${currentGame.scrambled}\n` + `Length: ${currentGame.word.length} letters\n` + `Attempts left: ${currentGame.maxAttempts - currentGame.attempts}\n` + `Type "hint" for a hint! (${currentGame.hintsGiven}/${currentGame.maxHints} used)`);
//   }
...
//    const { scrambled, length, timeLimit, maxAttempts } = await anagramGame.startGame(jid, selectedDifficulty);
//    activeGameJid = jid;

//    return await message.reply(`🎮 Anagram Game Started!\n` + `Difficulty: ${selectedDifficulty}\n` + `Unscramble this word: ${scrambled}\n` + `Length: ${length} letters\n` + `Time limit: ${timeLimit} seconds\n` + `Maximum attempts: ${maxAttempts}\n` + `Type "hint" for a hint!`);
//   } catch (error) {
//    console.error('Error starting game:', error);
//    return await message.reply('Sorry, failed to start the game. Please try again!');
//   }
//  }
// );
...
//  async (message) => {
//   const jid = message.chat;
//   if (jid !== activeGameJid) return;
//   if (!anagramGame.hasActiveGame(jid)) return;
//   if (!message.text) return;

//   if (message.text.toLowerCase().trim() === 'hint') {
//    const hintResult = anagramGame.getHint(jid);
//    return await message.reply(hintResult.message);
//   }
//   const result = anagramGame.checkGuess(jid, message.text);
//   if (['win', 'timeout', 'fail'].includes(result.status)) {
//    activeGameJid = null;
//   }
